...
export default function CardSwap({
  ...
  useEffect(() => {
    ...
    const centerShift = centerStack ? ((total - 1) * cardDistance) / 2 : 0;
    refs.forEach((r, i) => {
      if (r.current) {
        const slot = makeSlot(i, cardDistance, verticalDistance, total);
        placeNow(r.current, { ...slot, x: slot.x - centerShift }, skewAmount);
      }
    });
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cardDistance, verticalDistance, delay, pauseOnHover, skewAmount, easing, centerStack]);
